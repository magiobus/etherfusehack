...
const eventsLib = {
  ...
  attendeesSoldOut: async (db, eventId, inPerson = true) => {
    ...
    try {
      ...
      //in person check with limits
      let typeCheck = "attendeeLimit";
      if (!inPerson) typeCheck = "attendeeLimitVirtual";

      if (count >= event[typeCheck]) {
        return {
          soldOut: true,
          ticketsLeft: 0,
        };
      ...
      return {
        soldOut: false,
        ticketsLeft: event[typeCheck] - count,
      };
    } catch (error) {
