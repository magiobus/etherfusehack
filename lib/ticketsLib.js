...
const ticketsLib = {
  ...
  generateTicket: async (
    ...
    try {
      ...
      console.log("other instutition =>", otherInstitution);
      ...
      if (inPerson) {
        qr = await ticketsLib.generateQrCode({
          orderId,
          eventId,
          ticketType,
          user: {
        ...
        ticketImageText = ticketsLib.generateTicketImageText({
          userName: user.name,
          ticketQuantity,
          event,
          startTimeLocalText,
        ...
        qrUrl = await cloudinaryLib.uploadQrTicket(
          qr,
          orderId,
          ticketImageText
        );
      ...
      const ticket = {
        ...
        isMinor: isMinor ? isMinor : false,
        inPerson: inPerson ? inPerson : false,
        visitsFrom: visitsFrom ? visitsFrom : "",
        computerNeeded: computerNeeded ? computerNeeded : false,
        otherInstitution: otherInstitution ? otherInstitution : "",
      };
