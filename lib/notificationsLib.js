...
const notificationsLib = {
  ...
  async sendDiscordEmail(mailData) {
    ...
    const options = {
      ...
      html: `<h1>Hola ${user.name}!</h1> <p>Gracias por asistir al Etherfuse Hackathon!</p> <p>Para ingresar al server de discord, por favor ingresa al siguiente link: <a href="${discordLink}">${discordLink}<a/></p> </br> <p>Happy Hacking!</p>`,
    };
  ...
  async generateWhatsappTicketData({
    ...
    const waData = {
      ...
      template: "etherfuse_hackathon_registration_2",
      locale: "es_MX",
      components: [
        {
          type: "header",
